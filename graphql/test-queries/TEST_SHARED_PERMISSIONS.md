...
mutation CreateItemAsSharedUser {
  createItem(boardId: "1", name: "Item created by shared user", category: "Test") {
    id
    name
    category
    created_at
...
mutation UpdateItemAsSharedUser {
  updateItem(itemId: "ITEM_ID_HERE", name: "Updated by shared user") {
    id
    name
    updated_at
  }
...
mutation ShareBoardAsSharedUser {
  shareBoard(boardId: "1", email: "another@example.com", permission: VIEW) {
    id
  }
}
```
...
mutation ChangeToViewPermission {
  updateBoardShare(shareId: "SHARE_ID_HERE", permission: VIEW) {
    id
    permission_level
  }
}
...
mutation CreateItemAsViewUser {
  createItem(boardId: "1", name: "Should fail", category: "Test") {
    id
    name
  }
}
...
mutation ChangeToAdminPermission {
  updateBoardShare(shareId: "SHARE_ID_HERE", permission: ADMIN) {
    id
    permission_level
  }
}
